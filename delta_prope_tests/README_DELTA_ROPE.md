# Δ-RoPE (Delta-RoPE) Triton FlashAttention 实现

## 概述

本项目实现了基于 Triton 的 Δ-RoPE (Delta-RoPE) FlashAttention，这是一种选择性通道相对位置编码方法，专门为视频帧序列设计。该方法仅对指定的通道对应用基于帧差的 RoPE，其余通道使用标准注意力计算。

## 核心特性

- **选择性通道 RoPE**: 仅对指定的通道对应用 Δ-RoPE，其余通道正常计算
- **帧差索引**: 相位仅依赖帧差 Δ = t_j - t_i，无需绝对位置
- **融合计算**: Triton 内核中实现两路累加 (co/si) + LUT 融合 + 在线 softmax + 完整 @V
- **内存高效**: LUT 大小仅 [P, 2T-1]，支持在线 softmax
...
├── build_delta_rope_lut() # LUT 构建函数
├── pack_pairs_first()     # 通道重排函数
├── _delta_rope_flash_fwd() # Triton 内核
├── sdpa_delta_rope()      # 主接口函数
└── _reference_delta_rope_attention() # PyTorch 参考实现

test_delta_rope.py         # 基本功能测试
example_usage.py           # 使用示例和集成指南
...
### `sdpa_delta_rope(q, k, v, *, T, HW, rope_pairs_idx, rope_omega, is_causal=False, scale=None, block_m=64, block_n=64, block_dv=64)`
...
### `build_delta_rope_lut(omega, T, *, device=None, dtype=torch.float32)`

构建 Δ-RoPE 的余弦和正弦查找表。

**参数:**
- `omega`: `[P]` 需要应用 Δ-RoPE 的通道对对应的频率
- `T`: 最大帧数
- `device`: 设备 (可选)
- `dtype`: 数据类型 (默认 torch.float32)

**返回:**
- `cos_lut, sin_lut`: `[P, 2*T-1]` 查找表

### `pack_pairs_first(x, rope_pairs_idx)`

将需要应用 Δ-RoPE 的通道对移动到 head_dim 的前面。

**参数:**
- `x`: `[B,H,N,C]` 张量，C 必须为偶数
- `rope_pairs_idx`: `[P]` 需要应用 Δ-RoPE 的通道对起始索引
