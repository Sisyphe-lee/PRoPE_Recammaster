...
class LightningModelForTrain(pl.LightningModule):
    def __init__(
        ...
        self.ckpt_type = ckpt_type or ("wan21" if pipeline_type == "v2v" else "wan22")
        if self.ckpt_type not in {"wan21", "wan22"}:
            raise ValueError(f"Unsupported ckpt_type '{self.ckpt_type}'. Expected 'wan21' or 'wan22'.")
        self.expected_latent_channels = self._expected_latent_channels()
        ...
        self._validate_dit_input_channels()
        ...
        self.prompt_dropout_prob = 0.1 if pipeline_type == "i2v" else 0.0
    ...
    def training_step(self, batch, batch_idx):
        ...
        if latents.shape[1] != self.expected_latent_channels:
            raise RuntimeError(
                f"Latent 通道数为 {latents.shape[1]}，但当前 pipeline_type={self.pipeline_type} ckpt_type={self.ckpt_type} 期望 {self.expected_latent_channels}。"
                " 请检查 --dit_path 与 --tensor_suffix 是否匹配 (wan21 -> .tensors.pth / wan22 -> .wan22.tensors.pth)。"
            )
        ...
        if is_i2v and self.prompt_dropout_prob > 0 and torch.is_tensor(prompt_context):
            ...
            drop_mask = torch.rand(batch_size, device=self.device) < self.prompt_dropout_prob
            if drop_mask.any():
                uncond_context = self._get_uncond_context_for_batch(batch_size).to(
                    device=self.device, dtype=prompt_context.dtype
                )
                drop_mask = drop_mask.view(batch_size, 1, 1)
                prompt_context = torch.where(drop_mask, uncond_context, prompt_context)
                prompt_emb["context"] = prompt_context
        ...
        if is_i2v:
            ...
            if isinstance(self.frame_downsample_to, int) and self.frame_downsample_to > 0 and self.frame_downsample_to < tgt_latent_len:
                ...
                loss = torch.nn.functional.mse_loss(
                    noise_pred[:, :, 1:tgt_latent_len, ...].float(),
                    training_target[:, :, 1:tgt_latent_len, ...].float()
                )
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if latents.shape[1] != self.expected_latent_channels:
            raise RuntimeError(
                f"[val] Latent 通道数为 {latents.shape[1]}，但当前 pipeline_type={self.pipeline_type} ckpt_type={self.ckpt_type} 期望 {self.expected_latent_channels}。"
                " 请检查 --dit_path 与 --tensor_suffix 是否匹配 (wan21 -> .tensors.pth / wan22 -> .wan22.tensors.pth)。"
            )
        prompt_emb = copy.deepcopy(batch.get("prompt_emb", {}))
    ...
    def _expected_latent_channels(self) -> int:
        if self.pipeline_type == "i2v" and self.ckpt_type == "wan22":
            return 48
        return 16

    def _validate_dit_input_channels(self):
        model = self._get_denoising_model()
        patch = getattr(model, "patch_embedding", None)
        if patch is None or not hasattr(patch, "weight"):
            return
        in_ch = patch.weight.shape[1]
        expected = self.expected_latent_channels
        if in_ch != expected:
            raise RuntimeError(
                f"DiT patch_embedding 输入通道数为 {in_ch}，但当前 pipeline_type={self.pipeline_type} ckpt_type={self.ckpt_type} 期望 {expected}。"
                " 请确认 --dit_path 是否指向正确的 wan21/wan22 权重，并使 latent 后缀与 VAE 输出保持一致。"
            )
