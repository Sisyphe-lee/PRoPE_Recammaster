...
def _intersect_on_stamps(
    ...
    gt_map = {float(s): pose for s, pose in zip(gt_stamps.tolist(), gt_poses)}
    pr_map = {float(s): pose for s, pose in zip(pr_stamps.tolist(), pr_poses)}
    common = sorted(set(gt_map.keys()) & set(pr_map.keys()))
    if not common:
        raise ValueError("no overlapping frame ids between GT and predictions")
    poses_gt = np.stack([gt_map[s] for s in common], axis=0)
    poses_pr = np.stack([pr_map[s] for s in common], axis=0)
    stamps = np.array(common, dtype=np.float64)
...
def _compute_metrics(poses_gt: np.ndarray, poses_est: np.ndarray) -> Dict[str, float]:
    Pgt = poses_gt[:, :3, 3]
    Pest = poses_est[:, :3, 3]
    ...
    return {
        "ate_rmse": ate_rmse,
        "rte_rmse": _rmse(rpe_trans),
        "rre_rmse": _rmse(rpe_rot),
    }


def _evaluate_pair(name: str, gt_path: Path, pr_path: Path) -> TrajectoryResult:
    poses_gt, idx_gt = _load_pose_file(gt_path)
    poses_pr, idx_pr = _load_pose_file(pr_path)
    poses_gt, poses_pr, stamps = _intersect_on_stamps(poses_gt, idx_gt, poses_pr, idx_pr)

    aligned, info = _align_trajectories(poses_gt, poses_pr)
    metrics = _compute_metrics(poses_gt, aligned)
...
def main() -> int:
    ...
    for fname in common_names:
        ...
        try:
            res = _evaluate_pair(name, gt_files[fname], pr_files[fname])
            results.append(res)
